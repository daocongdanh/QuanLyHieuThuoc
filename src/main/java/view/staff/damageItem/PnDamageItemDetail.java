...
public class PnDamageItemDetail extends javax.swing.JPanel {
    ...
    public PnDamageItemDetail(Product product, List<UnitDetail> unitDetails, List<Batch> batchs, Promotion promotion,
            ...
            if (evt.getStateChange() == ItemEvent.DESELECTED) {
                ...
                if (parent != null) {
                    for (Component component : parent.getComponents()) {
                        if (component instanceof PnDamageItemDetail) {
                            PnDamageItemDetail pnOrderDetail = (PnDamageItemDetail) component;

                            if (pnOrderDetail.getProduct().equals(product) && !pnOrderDetail.getSelectedUnitDetail().equals(unitDetail)) {

                                List<UnitDetail> details = pnOrderDetail.getUnitDetails();
                                details.remove(unitDetail);
                                details.add(detailTmp);
                                pnOrderDetail.getComboChonDvt().removeItem(unitDetail);
                                pnOrderDetail.getComboChonDvt().addItem(detailTmp);
                ...
                for (Component component : pnListBatch.getComponents()) {
                    if (component instanceof PnSelectBatch) {
                        ...
                        pnSelectBatchs.add(new PnSelectBatch(batchDTO, unitDetail, spinnerSoLuong));
                    }
                }
...
//         TODO add your handling code here:
    ...
    private void btnXoaOderDetailMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnXoaOderDetailMouseClicked
        ...
        if (parent != null) {
            ...
            UnitDetail unitDetail = (UnitDetail) comboChonDvt.getSelectedItem();
            for (Component component : parent.getComponents()) {
                if (component instanceof PnDamageItemDetail pnOrderDetail) {
